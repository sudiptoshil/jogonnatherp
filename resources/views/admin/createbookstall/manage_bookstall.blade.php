    ...
    <div class="span12">
        ...
        <div class="widget orange">
            <div class="widget-title">
                <h4><i class="icon-reorder"></i> Manage All Bookstall User</h4>
            <span class="tools">
        ...
        <h3 align="center" style="color:red">{{Session::get('message')}}</h3>
            <div class="widget-body">
                <table class="table table-striped table-bordered table-advance table-hover">
                    ...
                    <tbody>
                        @php($i =1)
                        @foreach($bookstall as $v_admin)
                    <tr>
                    <td>{{$i++}}</td>
                    <td class="hidden-phone">{{$v_admin->name}}</td>
                    <td>{{$v_admin->email}}</td>
                    <td>{{$v_admin->admin->name}}</td>
                        ...
                        <td>
                            @if($v_admin->activity ==1)
                            <a href="{{route('bookstall-deactive',['id'=> $v_admin->id])}}" class="btn btn-success"><i class="icon-thumbs-down"></i></a>
                            @else
                            <a href="{{route('bookstall-active',['id'=> $v_admin->id])}}" class="btn btn-danger"><i class="icon-thumbs-up"></i></a>
                            @endif
                        </td>
                    </tr>
                    @endforeach
